...
function formatVerse(verse, glossingTerms) {
    ...
    let ret = `<li class="verse"><div class="flex"><p class="saffjahim-caliphatic">${verse["Saffjahim"]}</p><div class="inline"><p>${createLinkedVerse(verse)}</p><i>Plain:&Tab;${verse["Plain"]}<br>EME:&Tab;${verse["EME"]}<br>Literal:&Tab;${verse["Literal"]}</i>${formatInterlinearGloss(verse["Interlinear1"], verse["Interlinear2"], glossingTerms)}</div></div></li>`
    return ret
}
