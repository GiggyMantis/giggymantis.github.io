...
function syllabify(input, vowels) {
    ...
    const c_regex_plosives = "[pbtdkɡ]ʰ?";
    const c_regex_not_lr = "([^" + vowels + "lr\\.ː̯ʰ]ʰ?)"
    return input.replace(new RegExp(v_regex + c_regex_plosives + c_regex_not_lr + "(?=.)", "g"), "$1.$2$3").replace(new RegExp(v_regex + c_regex + c_regex + "(?=.)", "g"), "$1$2.$3").replace(new RegExp(v_regex + "(?=" + c_regex_unwrapped + "[" + vowels + "])", "g"), "$1.").replace(new RegExp(v_regex + v_regex_exclusive + "(?!̯)", "g"), "$1.");
}
